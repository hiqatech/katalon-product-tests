...
public class WebSteps {
	...
	def waitToDisAppear(String element) {
		...
		if(WebUI.waitForElementNotPresent(findTestObject(element), 5))
			...
			assert "element disappeared" == "false"
		}
	}
	...
	@Given("I open the browser on PEGA SDA3")
	def IOpenTheBrowserOnPEGASDA3() {
		CommonSteps.confData = findTestData("SDA3/Config/URLs")
		CommonSteps.confIndex = 1
		IOpenTheBrowserOnUrl(CommonSteps.getConfData("TestURL"))
		LoginTo(CommonSteps.getConfData("user"),CommonSteps.getConfData("passw"))
	}

	@Given("I open the browser on PEGA SSO SDA3")
	def IOpenTheBrowserOnPEGASSOSDA3() {
		CommonSteps.confData = findTestData("SDA3/Config/URLs")
	...
	@Given("I open the browser on PEGA DEV SDA3")
	def IOpenTheBrowserOnPEGADEVSDA3() {
		CommonSteps.confData = findTestData("SDA3/Config/URLs")
	...
	@Given("I open the browser on PEGA UAT SDA3")
	def IOpenTheBrowserOnPEGAUATSDA3() {
		CommonSteps.confData = findTestData("SDA3/Config/URLs")
		CommonSteps.confIndex = 7
		IOpenTheBrowserOnUrl(CommonSteps.getConfData("TestURL"))
		LoginTo(CommonSteps.getConfData("user"),CommonSteps.getConfData("passw"))
	}

	@Given("I open the browser on PEGA UAT SSO SDA3")
	def IOpenTheBrowserOnPEGAUATSSOSDA3() {
		CommonSteps.confData = findTestData("SDA3/Config/URLs")
		CommonSteps.confIndex = 8
		IOpenTheBrowserOnUrl(CommonSteps.getConfData("TestURL"))
		//LoginTo(getConfData("user"),getConfData("passw"))
	}

	@Given("I open the browser on PEGA DEV SDA2")
	def IOpenTheBrowserOnPEGASDA2() {
		CommonSteps.confData = findTestData("SDA2/Config")
		CommonSteps.confIndex = 1
		IOpenTheBrowserOnUrl(CommonSteps.getConfData("TestURL"))
		LoginTo(CommonSteps.getConfData("user"),CommonSteps.getConfData("passw"))
	}
	...
	def LoginTo(String user, String passw)
	{
		ITypeIntoTheElement(user, "PegaUI/PegaDevPage/user_name_field")
	...
	def ISelectTheElement(String element) {
		...
		if(element.contains("tab") || element.contains("page") ){
			...
			WebUI.sendKeys(findTestObject(element),  Keys.chord(Keys.PAGE_UP))
			Thread.sleep(waitTime)
			//WebUI.click(findTestObject("PEGAUI/CustomerPage/account_icon"))
			//Thread.sleep(waitTime)
		}
