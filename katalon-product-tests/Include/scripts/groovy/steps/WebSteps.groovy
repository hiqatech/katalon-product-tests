...
public class WebSteps {
	...
	def waitToAppear(String element) {
		...
		if(WebUI.waitForElementPresent(findTestObject(element), maxTimeOut)){
	...
	def waitToDisAppear(String element) {
		Thread.sleep(qickWait)
		if(WebUI.waitForElementNotPresent(findTestObject(element), maxTimeOut))
	...
	def waitToAppearIn(String element, String secs) {
		Thread.sleep(qickWait)
		if(WebUI.waitForElementPresent(findTestObject(element), secs as Integer)){
			moveToElement(element)
			WebUI.comment("Element " + element + " appeared")
		}
	...
	def waitToDisAppearIn(String element, String secs) {
		Thread.sleep(qickWait)
		if(WebUI.waitForElementNotPresent(findTestObject(element), secs as Integer))
			WebUI.comment("Element " + element + " disappeared")
		else assert "element disappeared" == "false"
	}
